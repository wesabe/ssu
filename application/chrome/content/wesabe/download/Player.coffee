...
wesabe.download.Player.register = (params) ->
  @create params, (klass) ->
    # make sure we put it where wesabe.require expects it
    wesabe.provide "fi-scripts.#{params.fid}", klass

wesabe.download.Player.create = (params, callback) ->
  klass = ->
    # inherit from Player
    wesabe.lang.extend this, wesabe.download.Player::, override: false
  ...
  klass.fid = klass::fid = params.fid
  # the name of the Financial Institution (e.g. Wells Fargo)
  klass.org = klass::org = params.org
  # ofx info in case this is a hybrid
  klass.ofx = params.ofx

  klass::canHandleGoal = params.canHandleGoal or (-> true)
...
wesabe.download.Player::start = (answers, browser) ->
  if @userAgent
...
wesabe.download.Player::nextGoal = ->
  @job.nextGoal()

wesabe.download.Player::onLastGoalFinished = ->
  wesabe.info 'Finished all goals, running callbacks'
  for callback in @afterLastGoalCallbacks
    @runAction callback, @browser, @page

wesabe.download.Player::finish = ->
  @clearErrorTimeout 'action'
  @clearErrorTimeout 'global'
  @clearErrorTimeout 'security'

wesabe.download.Player::runAction = (name, browser, page, scope) ->
...
wesabe.download.Player::resume = (answers) ->
...
wesabe.download.Player::getActionProxy = (browser, page) ->
  new ActionProxy this, browser, page

wesabe.download.Player::getJobProxy = ->
  @job

wesabe.download.Player::download = (url, metadata) ->
  newStatementFile = =>
    folder = wesabe.io.dir.profile
    folder.append 'statements'
...
#
wesabe.download.Player::answerSecurityQuestions = ->
...
# @public
#
wesabe.download.Player::fillDateRange = ->
...
wesabe.download.Player::nextAccount = ->
  delete tmp.account
  reload()


wesabe.download.Player::skipAccount = (args...) ->
  wesabe.warn(args...) if args.length
  delete @tmp.account

wesabe.download.Player::actionTimeoutDuration = 60000 # 1m
wesabe.download.Player::globalTimeoutDuration = 300000 # 5m
wesabe.download.Player::securityTimeoutDuration = 180000 # 3m

wesabe.download.Player::setErrorTimeout = (type) ->
...
wesabe.download.Player::clearErrorTimeout = (type) ->
  if @_timeouts?[type]
    wesabe.debug "Timeout ", type, " cleared"
    clearTimeout @_timeouts[type]

wesabe.download.Player::onDocumentLoaded = (browser, page) ->
...
wesabe.download.Player::triggerDispatch = (browser, page) ->
  module = @constructor.fid
...
wesabe.download.Player::onDownloadSuccessful = (browser, page) ->
  for callback in @afterDownloadCallbacks
    @runAction callback, browser, page

wesabe.download.Player::shouldDispatch = (browser, page) ->
...
wesabe.download.Player::__defineGetter__ 'history', ->
  @_history ||= []

wesabe.download.Player::__defineGetter__ 'tmp', ->
  @_tmp ||= {}

wesabe.download.Player.build = (fid) ->
