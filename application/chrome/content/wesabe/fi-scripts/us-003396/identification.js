wesabe.provide('fi-scripts.us-003396.identification', {
  dispatch: function() {
  ...
  actions: {
    acknowledgeIdentificationRequirement: function() {
      page.click(e.identification.continueButton);
    },

    jumpToIdentificationCodeEntry: function() {
      page.click(e.identification.alreadyHaveIdentificationCodeLink);
    },

    collectIdentificationCodeDeliveryMethods: function() {
      tmp.identificationCodeDeliveryMethods = [];

      log.debug("Collecting phone delivery method options");
      var phoneContainer = page.find(e.identification.contacts.phone.container);
      if (!phoneContainer) {
        log.warn("No phone number container -- maybe failed to find it?");
      } else {
        var options = page.select(e.identification.contacts.option, phoneContainer);
        log.debug("Found phone delivery options: ", options);
        options.forEach(function(option) {
          option = wesabe.untaint(option);

          var mask = wesabe.untaint(page.findStrict(e.identification.contacts.phone.mask, option));
          var method = {value: option.value, key: option.id, label: mask.nodeValue};
      ...
      var emailContainer = page.find(e.identification.contacts.email.container);
      if (!emailContainer) {
        log.warn("No email container -- maybe failed to find it?");
      } else {
        var options = page.select(e.identification.contacts.option, emailContainer);
        log.debug("Found email delivery options: ", options);
        options.forEach(function(option) {
          option = wesabe.untaint(option);

          var mask = wesabe.untaint(page.findStrict(e.identification.contacts.email.mask, option));
          var method = {value: option.value, key: option.id, label: "Email "+mask.nodeValue};
    ...
    chooseIdentificationDeliveryMethod: function() {
      if (answers.identificationCodeDeliveryMethod) {
        var option = page.find(wesabe.xpath.bind(e.identification.contacts.optionTemplate, {value: answers.identificationCodeDeliveryMethod}));
        if (!option) {
    ...
    confirmIdentificationCode: function() {
      page.click(e.identification.continueButton);
    },

    enterIdentificationCode: function() {
      if (answers.identificationCode) {
